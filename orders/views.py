...
def place_order(request):
    ...
    k = {}
    for i in cart_items:
        productitem = ProductModel.objects.get(pk=i.productitem.id, vendor__id__in=vendors_ids)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.total_data = json.dumps(total_data) # ADDED IN 221
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        #  STORE THE PAYMENT DETAULS IN THE PAYMENT MODEL BACKEND
        ...
        for item in cart_items:
            ...
            ordered_food.product_item = item.productitem
            ordered_food.quantity = item.quantity
            ordered_food.price = item.productitem.price
            ordered_food.amount = item.productitem.price * item.quantity # total amount
            ordered_food.save()
        ...
        # SEND ORDER CONFIRMATION EMAIL TO THE VENDOR or MULTIPLE VENDOR
